...
```text
cn_futures_daily_trend_offline.yaml
```

它自动下载商品期货主力连续合约日线数据并运行趋势信号，且明确设置
...
运行 `uv run northstar data download --profile cn_futures_daily_trend_offline` 会自动下载并写入
...
3. `futures`、`data` 和 `data.download` 的合约规格、连续合约、数据提供器、路径和价格口径。
...
uv run northstar data download --profile <你的期货画像_offline>
uv run northstar data validate --profile <你的期货画像_offline>
uv run northstar research futures-trend --profile <你的期货画像_offline>
```
