...
按下面顺序完成首次上手，而不要从长篇专题文档中随机跳读：

1. 阅读第 1 节，确认项目当前没有真实 CTP 或实盘能力；
2. 跟随第 2 节初始化并运行项目；
3. 阅读[架构设计](ARCHITECTURE.md)的安全原则、依赖方向和所属领域；
4. 以[开发与研究工作流](DEVELOPMENT.md)作为初始化、质量门禁和 migration 的唯一操作规范；
5. 再按第 6 至 8 节完成自己的第一个小改动。

本指南是学习路径。任何易变的操作、配置、运行或治理事实与专题文档不一致时，专题权威优先。
...
- production YAML；
- 真实账户、真实资金或实盘交易能力；
- 已授权的生产数据、PostgreSQL Contract Authority 发布和 Calendar ArtifactSnapshot；
- 已批准的生产灾备环境。
