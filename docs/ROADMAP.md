...
| 检查 | 已验证结果 | 边界 |
|---|---|---|
| 完整后端 | `a6fcde9`：959 项通过，无跳过 | 本机 PostgreSQL/SQLite；后续改动另行回归 |
| 类型与协议 | 279 个 Python 源文件 Mypy、Protobuf 生成一致性通过 | 不替代经济事实或运行验收 |
| 前端 | 21 项测试、TypeScript 检查、三个 Next.js 构建通过 | 已覆盖“业务 UNKNOWN”与“命令未确认”的区别 |
| 三工作台安装页面 | `2fea71a` wheel＋`e4142fc` 前端：三工作台通过，浏览器错误为零 | 固定预算引用、未知条件不锁操作、旧查询不被替换；合成 HTTP，无柜台连接 |
...
3. **#49/#32**：在实际接收核心中连接当前账户/行情/风险检查、运行授权、持久订单/预占和原生报撤单通道；当前已接通显式授权的一手 SimNow 空仓开仓与原通道撤单；仍需完整持仓后的平仓、持续策略执行与实际柜台闭环。
