...
安装浏览器验收发现 `a0e5063` 对数据库返回的上海时区时间过严拒绝。已改为绑定时归一化有时区的时间，
保留对无时区时间的拒绝；等价时区产生相同计划身份，跨日/SQLite 回归 11 项通过，等待新安装包重验。
Portfolio 估值收回既有 `accounting/portfolio.py`，删除独立 valuation 文件，保持一个当前入口。

`ffa4c49` 的新安装包通过三工作台浏览器复验，包含固定评价条件、后台任务独立执行，浏览器错误为零。
Live 本地账户/实例/接收锁及账户目录被替换时，接收线程现在独立检测并停止，不依赖网页访问；
保留已提交的回报和处理游标，不自动重连或重新获取账户所有权。SQLite 故障及 Live 相关测试 87 项通过。

继续将每张模拟订单的手续费/开仓保证金预算固定到订单身份与检查点，Execution 按剩余量保留预占，
平仓按已固定开平与今昨仓语义占用数量，不提前花费预计释放的保证金。Research/Paper 与报告已接通；
部分成交减少残量预占，撤销替换不重复占用，预占不改变已发生费用或权益。真实 PostgreSQL/SQLite 路径
25 项及研究/模拟 16 项检查通过。此项不代表 Live 已具备授权、持久发送尝试或柜台冻结资金核对。

本地完整后端回归 676 项通过，包含真实 PostgreSQL 联合备份恢复；232 个源文件 Mypy、Ruff、
协议生成检查、前端类型检查/14 项测试和三个前端构建通过。整批仍未完成，不推送。
