...
2026-09-13：#18 已将 1/5/15/30/60 分钟周期贯穿现有规范化、质量、固定快照、离线语义及 Research/Paper 输入；保留默认 1m 的内部研究声明，发布后周期不可变。五周期发布/重读、提前可得拒绝与 15 分钟持久回放对账通过本地 PostgreSQL 验证。尚未完成 Tushare 原始响应到真实跨日清单的自动装配；不推断供应商时间标签，不接受跨休市或不完整时段作为完整 Bar。
