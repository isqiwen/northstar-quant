...
`aa00494` 安装包通过三个工作台浏览器、后台任务/管理端生命周期隔离验收，含订单预占列，浏览器错误为零。
继续按固定涨跌停条款补齐方向性排队限制，已有成交量也不虚构涨停买入/跌停卖出的队列优先级；
订单保持未成交与预占，相关研究/模拟检查 20 项通过。仍未连接柜台。

候选材料改用 `same_clean_revision` 明确代码来源一致性，删除旧生产资格命名；Live 材料接收仍不授予交易。
候选/协议/SQLite 接收与恢复、固定跨日研究相关检查 29 项通过，232 个源文件类型检查及前端构建通过。

三工作台接通独立操作者登录（#26）：固定成本密码摘要、服务端限时会话、管理 API 身份/CSRF 检查、
注销与重启撤销、前端登录门禁；后台 worker/交易内核不参与登录。部署自动初始化随机密码，保留已安装身份，
显式新摘要才更换密码。未知命令只缓存核查身份，不再缓存请求体或 Tushare token。
后端全量回归 682 项通过后修正重启会话测试，相关 7 项复核通过；234 个源文件类型检查、Ruff、
协议生成、前端 16 项测试和三个构建通过。安装与浏览器验证继续进行，整批仍未完成，不推送。

`e6484d4` 后端安装包通过完整安装、联合备份恢复和六个日志写入进程的隔离验收。
配合本地前端修正，通过三工作台浏览器登录/退出、数据浏览、研究报告和任务生命周期验收，浏览器错误为零；
Research 任务在前端/API/Data Hub 停止后完成。Live 检查仍为合成控制响应，无柜台连接。
登录页按应用标识；Data Hub 在登录前注册协议。浏览器验收每次重置明确指定的临时测试数据库，
避免失败重跑混入旧收据；不清理个人部署或业务数据。继续 #49 持久授权与订单预占，未推送。

Live 命令当前已固定可信操作人并修复真实 Protobuf 回执查询；41 项 PostgreSQL/SQLite 与 HTTP 检查通过，包含重启查询、重复命令、伪造浏览器身份与缺失内部身份拒绝。内部协议同步更新为 3；仍未创建柜台发送授权或连接柜台，整批保持本地进行中。

命令身份/终态已增加数据库写入约束，SQLite 故障与事实保留检查 14 项通过。完整后端检查中 675 项通过；因本次测试进程缺少 PostgreSQL 客户端 PATH 导致的 1 项失败与 7 项跳过，补齐 PATH 后联合恢复及相关 Live/HTTP 共 27 项通过。未操作个人部署。
