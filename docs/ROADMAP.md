...
| #48/#49 运行边界 | 候选/账号唯一锁、当前实例 owner 执行限额、撤销、预占、有界 CTP 私有通道；UNKNOWN 不重发 | 许可与当前账户/市场/风险的最终发送接入，独立告警 |
...
| 检查 | 已验证结果 | 边界 |
|---|---|---|
| 完整后端 | `bd186e8`：970 项通过，无跳过 | 本机 PostgreSQL/SQLite；后续改动另行回归 |
...
| 三工作台安装页面 | `bd186e8` wheel＋前端：三工作台通过，浏览器错误为零 | 固定预算及实际开仓表单、未知条件不锁操作；合成 HTTP，无柜台连接 |
...
| 联合恢复 | `298d751` 新 wheel：固定行情、研究重放、空库恢复、来源和六类日志通过 | 私有 `contract-298d751` 记录安装身份、数据库版本、存储 UUID；未操作个人主机 |
...
3. **#49/#32**：在实际接收核心中连接当前账户/行情/风险检查、运行授权、持久订单/预占和原生报撤单通道；当前已接通显式授权的一手 SimNow 空仓开仓与原通道撤单；已接通同一接收核心的一手平今仓，合成开平往返与费用未知保留通过；当前平仓页面/恢复回归继续，持续策略执行与实际柜台闭环仍待完成。
