...
个人使用入口按数据、研究、账户、运行与对账组织，不要求使用者拼接内部步骤。
...
| Session | 从固定配置创建会话、推进事实、查询和控制 | PostgreSQL 事务、游标、重建、输入新鲜度、生命周期；Paper 与柜台会话分别记录资金事实 | 待实现；不提前建立空包 |
| Broker execution | 只读核对；执行已授权委托；接收回报；恢复未决状态 | 具体柜台 Adapter、订单状态、预占、发送尝试、节流、独占外发与 UNKNOWN 对账 | 待实现；先选一个真实接口，不建网关注册框架 |
| Workspace | 导入、研究、账户核对、会话启用与控制 | 用户会话、命令授权与去重、状态呈现；禁止网页直接调用柜台 | `web.py`、`static/`、`cli.py` |
...
随着对应功能落地，再增加会话/输入处理、执行授权/发送尝试、
订单/成交、账户分录/结算、控制命令等实际所需记录。
关系和唯一约束属于拥有行为的数据库模型；灵活研究报告继续用 JSONB。
不为文档先建表，不构建覆盖全系统的事件溯源框架，也不保存两份可独立修改的账户真相。

网页围绕数据、研究、账户、运行和需要处理的对账问题组织，始终醒目标记真实/仿真环境。
默认显示合约、日期、来源、状态、净值和原因，UUID 与摘要进入详情。
允许从已有数据继续研究或从固定配置启动会话；运行实盘还必须满足单独的执行授权。
状态页区分数据库、行情/交易连接、数据新鲜度、账户核对、未决委托与执行权限，
不能把当前仅检查数据库的 `/health/ready` 当成“可交易”。
