...
用户要求自行选择免费国内期货数据。首选 **Shinny EDB 具体合约分钟线**，
辅以上期所公开日行情和每日结算参数；不使用连续合约代替真实合约。
2026-09-09 核对 [EDB 官方文档](https://doc.shinnytech.com/edb/latest/md_server.html)：
免 token 可查询近一年分钟线与历史日线，分钟线更早历史属于付费范围，不接入付费权限。
[上期所日周数据](https://www.shfe.cn/reports/tradedata/dailyandweeklydata/)用于官方参考核验。
新浪具体合约分钟接口仅保留候选，不新增第二套同步采集实现。

当前已接通 `data collect-edb` 单次有界下载与原文加工，见 [CLI](CLI.md)。
尚未实现计划、水位和自动补数，也不承诺完整覆盖或服务可用性。
采集须固定真实合约、范围与内容身份，限速、保留原文，缺口显式记录；下载时间不冒充历史首次可得时间。
来源使用范围不等于公开再分发授权，默认不公开发布下载原文。

本轮无凭据读取 `SHFE.rb2610` 的 2026-09-08 09:00–15:00（北京时间），HTTP 成功，
返回 225 根分钟线、13,316 字节，时间戳递增且无重复。
内容 SHA-256：`237583a17302d7db17cb650549435c76bb7a83b1e6dee1ca5537650dbcb47986`。
仅核对响应结构与时间顺序，未宣称完整质量/跨日结算验收；原文未提交到公共仓库。

同日通过新 `collect-edb` 命令读取 2026-09-08 13:30–15:00：5,361 字节、90 根分钟线，
SHA-256 `11802406e30c54bca8c654205d4c89b1563f2c50e9af8cc4cbc39185fe073a36`。
隔离 PostgreSQL 中先返回 PENDING，再由独立 worker 发布；90 条全部接受，零缺失、重复和冲突。
本次为开发源码外部小范围验收，不代表持久自动下载、夜盘或跨日回测；原文没有公开提交。
