...
第一轮按获取方式分为外部文件导入与行情自采。历史文件在系统外获取，Data Hub 负责留存、校验与发布；
Recorder 在 core 独立持续积累自采资产。具体自采接口、使用范围与外部接入验收仍须落实，不能用 Live 的有界接收代替。

已有 **Shinny EDB 具体合约分钟线** 有界下载保留为可选适配，上期所公开日行情和每日结算参数作参考；
不继续把多供应商自动下载作为第一轮目标，不使用连续合约代替真实合约。
