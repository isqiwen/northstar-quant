...
协议仍由 `proto/data_hub.proto` 定义；Research 后端无需 token 即可获取清单，不获取 core 数据库凭据。
Docker 仅通过 core 私网 `19090` 的只读网关开放这些 GET；`19082` 管理 API 保持本机绑定。
Research 按清单校验 发布目录中的文件，在本机 DuckDB 查询；不扫描目录拼接新旧版本。网络或文件校验失败明确报错。
