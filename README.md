...
打开 Data Hub → **历史同步**，保存 token 后点击 **更新品种目录**，选择品种再点击 **探查并下载所选品种**。后台按真实合约上市首段逐数据集探查，跳过历史核心不完整的合约，完整生命周期核验通过后发布。无固定年份下界；未选择品种只更新目录。
...
Data Hub 从 2012-01-01 起按最后交易日正序查找已结束真实合约；最后交割日也须已完成。
跨越起点的合约仍保留真实上市日起的完整行情，元数据日期不明则等待核实。
