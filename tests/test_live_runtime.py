...
def test_live_auth_expiry_and_target_rejection_have_no_account_effect(
    ...
    with TestClient(app) as http:
        ...
        headers = {
            ...
            "X-Northstar-Release": release_hash(),
        ...
        headers["X-Northstar-Release"] = "another-release"
        assert http.post(path, json=body, headers=headers).status_code == 409
        headers["X-Northstar-Release"] = release_hash()
...
def test_interrupted_effect_stays_unknown_and_old_page_never_retargets_new_live(
    ...
    library = DataLibrary(postgres_engine, SourceFiles(tmp_path))
    app = create_app(postgres_engine, library, auth)
    original = app.state.owner.broker.establish_baseline
    attempts = 0
    ...
    monkeypatch.setattr(app.state.owner.broker, "establish_baseline", interrupt_after_commit)
    identifier = uuid4()
    with TestClient(app) as http:
        client = LiveClient("http://localhost", auth, client=http)
