...
def test_all_post_bootstrap_uv_run_routes_are_non_sync_offline() -> None:
    for path in (JUSTFILE, CI_WORKFLOW):
        for raw_line in path.read_text(encoding="utf-8").splitlines():
            line = raw_line.strip()
            if line.startswith("uv run ") or line.startswith("run: uv run "):
                assert "uv run --offline --no-sync " in line

    dev_setup = DEV_SETUP.read_text(encoding="utf-8")
    assert '"uv", "sync"' not in dev_setup
    assert '"uv", "run", "--offline", "--no-sync"' in dev_setup
    ...
    assert re.search(r'"run",\s+"--offline",\s+"--no-sync",', tasks) is not None
