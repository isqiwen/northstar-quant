...
from northstar_quant.foundation.reporting import pdf_renderer
...
def test_generic_report_builder_rejects_backtest_report_type(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="build_backtest_report"):
        report_builder.build_markdown_report(
...
def test_generic_report_builder_rejects_manual_formal_backtest_manifest():
    """正式回测清单只能从已验证的 BacktestRun 投影进入报告层。"""
    ...
    with pytest.raises(ValueError, match="build_backtest_report"):
        report_builder.build_markdown_report(
            ...
            metrics={},
            profile_id="cn_futures_daily_trend_offline",
            backtest_run={"run_id": "bt-handmade"},
        )
