...
from northstar_quant.foundation.db.models import AccountAttributionRecord, AnomalyEventRecord
from northstar_quant.application import reporting as report_builder


def test_daily_report_includes_latest_account_attribution(
    tmp_path,
    monkeypatch,
...
def test_daily_report_emits_alert_lines_when_thresholds_are_breached(
    tmp_path,
    monkeypatch,
    postgresql_session_factory,
):
    ...
    assert subject == (
        "Northstar Quant - 日报 [执行异常][账本异常] "
        "- daily__cn_futures_daily_live__portfolio__2024-03-04"
    )


def test_daily_report_emits_funding_alerts_for_large_cash_flows(
    tmp_path,
    monkeypatch,
    postgresql_session_factory,
):
...
def test_record_daily_anomaly_events_is_idempotent(
    tmp_path,
    monkeypatch,
    postgresql_session_factory,
):
