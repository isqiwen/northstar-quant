...
def test_empty_position_batch_supersedes_previous_non_empty_batch(tmp_path):
    engine = create_engine(
        postgresql_test_url(tmp_path / "empty-position-batch.db"),
        future=True,
    )
    Base.metadata.create_all(bind=engine)
...
def test_latest_positions_are_isolated_by_broker_account_and_profile(tmp_path):
    engine = create_engine(
        postgresql_test_url(tmp_path / "scoped-position-batches.db"),
        future=True,
    )
    Base.metadata.create_all(bind=engine)
