...
def test_repository_script_and_infrastructure_layout_is_explicit() -> None:
    ...
    assert '"127.0.0.1:${POSTGRES_PORT:-5432}:5432"' in compose_file.read_text(
        encoding="utf-8"
    )
...
def test_runtime_output_paths_are_configurable_and_consistent() -> None:
    ...
    assert 'SYSTEMD_TEMPLATE_DIR="${DEPLOYMENT_ROOT}/infra/systemd"' in release_script
    assert 'tar -C "${ROOT_DIR}" -czf - scripts/deploy infra/systemd |' in deploy_script
...
def test_private_dashboard_deploy_is_explicit_opt_in_and_keeps_main_service_mode() -> None:
    ...
    release_script = (DEPLOY_DIR / "install-release.sh").read_text(encoding="utf-8")
    config_loader = (DEPLOY_DIR / "lib" / "config.sh").read_text(encoding="utf-8")
    dashboard_template = (SYSTEMD_DIR / "dashboard.service.in").read_text(
        encoding="utf-8"
    )
