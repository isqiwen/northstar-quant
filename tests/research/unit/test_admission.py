...
def test_admission_rejects_hand_declared_pit_safety_without_a_guard_certificate():
    profile = load_trading_profile("cn_futures_daily_actual_offline")
    policy = replace(
    ...
    assert result.eligible_for_human_review is False
    point_in_time_check = next(
        item for item in result.checks if item.check_id == "data.point_in_time_decision_safety"
    )
    assert point_in_time_check.status == "FAIL"
    assert "LookaheadCertificate" in point_in_time_check.message
...
def test_static_as_of_view_is_explicitly_blocked_from_candidate_admission():
    profile = load_trading_profile("cn_futures_daily_actual_offline")

    result = evaluate_research_admission(
        profile,
        source_manifest={
            "governance": {"source_id": profile.data.source_id},
            "point_in_time": {
                "selection_mode": "STATIC_AS_OF_VIEW_NOT_DECISION_REPLAY",
                "decision_time_safe": False,
            },
        },
        raw_market_df=pl.DataFrame({"date": [date(2026, 3, 27)], "product": ["RB"]}),
        equity_curve=[{"date": "2026-03-27", "equity": 1.0}],
        performance={},
        execution={},
    )
