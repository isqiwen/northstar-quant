...
from tests.helpers.futures_actual import (
...
def test_ctp_sim_requires_simulated_executable_profile():
    ...
    assert simulated.calendar == "CN_FUTURES"
    assert simulated.futures is not None
    assert simulated.futures.calendar_artifact_snapshot_hashes == {}
...
def test_profile_boolean_strings_are_parsed_by_value(tmp_path: Path):
    ...
    assert profile.futures is not None
    assert profile.futures.calendar_artifact_snapshot_hashes == {
        "SHFE": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    }


def test_uncached_profile_load_observes_calendar_artifact_pin_revocation_immediately(
    tmp_path: Path,
):
    """逐订单门禁不能因进程画像缓存继续使用已被替换的制品 pin。"""
    ...
    revoked_hash = "b" * 64
    try:
        cached = load_trading_profile(profile_id, tmp_path)
        assert cached.futures is not None
        assert cached.futures.calendar_artifact_snapshot_hashes == {"SHFE": old_hash}

        path.write_text(
            path.read_text(encoding="utf-8").replace(old_hash, revoked_hash),
            encoding="utf-8",
        )

        # 普通 loader 有意缓存；最后订单门禁使用的 uncached API 必须立刻观察文件变更。
        assert load_trading_profile(profile_id, tmp_path).futures is not None
        assert (
        ...
        refreshed = load_trading_profile_uncached(profile_id, tmp_path)
        assert refreshed.futures is not None
        assert refreshed.futures.calendar_artifact_snapshot_hashes == {"SHFE": revoked_hash}
    finally:
        load_trading_profile.cache_clear()
