...
from northstar_quant.application.cli import app
...
def test_live_trade_attribution_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.recent_trade_attributions", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
...
def test_live_account_attribution_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.recent_account_attributions", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
...
def test_live_anomaly_events_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.recent_anomaly_events", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
...
def test_live_preflight_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.run_live_preflight", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
...
def test_live_shadow_run_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.run_shadow_once", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
...
def test_live_run_health_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.recent_run_health", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
...
def test_live_soak_summary_command_logs_payload(monkeypatch):
    ...
    monkeypatch.setattr("northstar_quant.application.cli.soak_summary", lambda **_: payload)
    monkeypatch.setattr(
        "northstar_quant.application.cli._log_json",
        lambda data, **context: captured.update({"data": data, "context": context}),
    )
