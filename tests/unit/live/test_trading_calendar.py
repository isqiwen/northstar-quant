...
def test_last_trading_session_of_year_uses_exchange_calendar():
    assert trading_calendar.is_last_trading_session_of_year(
        datetime(2024, 12, 31, 15, 0),
        calendar="XSHG",
        timezone="Asia/Shanghai",
    )
    assert not trading_calendar.is_last_trading_session_of_year(
