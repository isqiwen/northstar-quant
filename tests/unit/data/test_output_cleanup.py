...
def test_cleanup_dry_run_then_apply_only_removes_expired_download_cache_and_safe_tmp(
    ...
    provider_cache = downloads_dir / "akshare_actual_daily" / "market_bars" / "SHFE" / "bars.parquet"
    ...
    assert all(path.exists() for path in (primary_cache, cache_manifest, provider_cache, temporary_file))
