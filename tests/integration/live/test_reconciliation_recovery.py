...
def test_reconcile_recovers_completed_order_and_cancel_terminal_states(postgresql_engine):
...
def test_reconcile_does_not_finalize_cancel_from_non_terminal_order_state(postgresql_engine):
    engine = postgresql_engine
    asof = datetime(2026, 7, 23, 2, 30, tzinfo=UTC)
    snapshot = BrokerStateSnapshot(
        open_orders=[
