...
def test_save_fill_snapshots_creates_buy_trade_attribution(postgresql_engine):
...
def test_save_fill_snapshots_creates_sell_trade_attribution_with_correct_sign(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
...
def test_save_fill_snapshots_uses_broker_execution_identity(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
...
def test_fill_identity_uses_perm_id_before_reused_order_id(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
...
def test_partial_fill_does_not_downgrade_cancelled_terminal_order(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
...
def test_existing_unlinked_fill_is_backfilled_by_order_ref(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
...
def test_fill_ledger_does_not_reduce_completed_cumulative_progress(postgresql_engine):
    engine = postgresql_engine
    filled_at = datetime(2024, 3, 4, 15, 36, tzinfo=UTC)
