...
def main() -> None:
    ...
    service = json.loads(result.stdout)["services"]["storage-check"]
    env = service["environment"]
    mount = str(Path(service["volumes"][0]["source"]).parent)
    expected = {
        "server": env["NORTHSTAR_NAS_ADDRESS"],
        "export": env["NORTHSTAR_NFS_EXPORT"],
        "version": env["NORTHSTAR_NFS_VERSION"],
        "mount": mount,
        "mode": "rw" if args.app == "data_hub" else "ro",
    }
    try:
