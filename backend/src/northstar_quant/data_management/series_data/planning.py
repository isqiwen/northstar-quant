"""Product-selected, bounded independent collection through the shared supplier queue."""
...
def plan(engine: Engine) -> None:
    ...
    with engine.begin() as c:
        ...
        for r in c.execute(
            ...
            start = metadata_date(r["details"].get("list_date"))
            if start is None:
                continue
        ...
        if (
            ...
            AND EXISTS
            (SELECT 1 FROM data_series_requests s JOIN data_series_collections sc
                ON sc.dataset=s.dataset AND sc.scope=s.scope WHERE s.request_id=j.request_id
                AND sc.exchange || ':' || sc.product=ANY(
                    (SELECT selected_products FROM data_sync_settings)::text[]))"""),
            )
            >= 64
        ):
