    ...
    connection.exec_driver_sql("""
        ...
        BEGIN
            ...
            IF (NEW.attempt_id, NEW.source_id, NEW.request_id, NEW.request_hash,
                NEW.processing_hash, NEW.code_revision, NEW.parameters,
                NEW.retry_of, NEW.created_at)
                IS DISTINCT FROM
                (OLD.attempt_id, OLD.source_id, OLD.request_id, OLD.request_hash,
                OLD.processing_hash, OLD.code_revision, OLD.parameters,
...
class DataLibrary:
        ...
        processing_hash = _digest(
            {
                ...
                **(
                    {"development_attempt": request_id} if implementation.endswith("-dirty") else {}
                ),
            }
        )
        with self._engine.connect() as connection:
    ...
    def _stream_document(self, content: bytes) -> dict[str, object]:
        from northstar_quant.broker.stream_records import read_stream_archive
