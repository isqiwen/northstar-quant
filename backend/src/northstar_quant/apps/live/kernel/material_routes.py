...
def routes(owner: LiveOwner, engine: Engine, library: DataLibrary) -> APIRouter:
    ...
    router = APIRouter()
    materials = StrategyMaterials(engine)

    @router.get("/strategy-materials")
    def candidates() -> list[dict[str, Any]]:
        return materials.list()

    @router.post("/strategy-materials")
    def receive(request: Request, body: dict[str, Any]) -> dict[str, Any]:
        return execute_command(owner, request, body, lambda identifier: materials.accept(body))
