...
def test_paper_restarts_with_fixed_configuration_and_matches_batch_account(
    ...
    assert ConfigurationStore(postgres_engine).save_configuration("日盘基线", config) == saved
    session_id = uuid4()
    ...
    assert created["summary"]["ending_cash"] == config.simulation.to_dict()["initial_cash"]
    ...
    newer = ConfigurationStore(postgres_engine).save_configuration(
        "日盘基线", replace(config, risk=replace(config.risk, max_lots=config.risk.max_lots + 1))
    )
    ...
    try:
        restored = PaperStore(reopened, library)
...
def test_failed_commit_lost_ack_and_competing_workers_never_duplicate_money(
    ...
    library, dataset, config = _study(postgres_engine, tmp_path)
    store = PaperStore(postgres_engine, library)
    saved = ConfigurationStore(postgres_engine).save_configuration("transaction check", config)
    identity = uuid4()
    ...
    assert (
        PaperStore(postgres_engine, library).advance(identity, request_id=command)
        == acknowledged_later
    )
    same_command = uuid4()
    with ThreadPoolExecutor(max_workers=2) as workers:
        ...
        duplicate = workers.submit(
            PaperStore(postgres_engine, library).advance, identity, request_id=same_command
        )
...
def test_changed_implementation_or_corrupted_checkpoint_cannot_resume(
    ...
    library, dataset, config = _study(postgres_engine, tmp_path)
    store = PaperStore(postgres_engine, library)
    saved = ConfigurationStore(postgres_engine).save_configuration("identity check", config)
    identity = uuid4()
    store.create(dataset.snapshot_id, str(saved["configuration_id"]), request_id=identity)
    store.advance(identity, request_id=uuid4())
    with monkeypatch.context() as presentation_change:
        presentation_change.setattr(
            "northstar_quant.research.paper.code_revision", lambda: "b" * 40
        )
        assert store.get(identity)["can_advance"] is True
        advanced = store.advance(identity, request_id=uuid4())
        assert advanced["step"]["code_revision"] == "b" * 40
    with monkeypatch.context() as changed:
        changed.setattr("northstar_quant.research.backtesting.TradingSession.REVISION", "changed")
        assert store.get(identity)["can_advance"] is False
        with pytest.raises(ValueError, match="computation and checkpoint"):
            store.advance(identity, request_id=uuid4())
        assert store.get(identity)["cursor"] == 2
    with postgres_engine.begin() as connection:
        connection.execute(
            text(
