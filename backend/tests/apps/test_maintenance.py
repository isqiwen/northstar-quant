...
def test_initialization_and_restore_keep_all_interrupted_query_evidence(
    ...
    order_check = OrderReviews(postgres_engine).check(position_check_id, request_id=order_check_id)
    funds_id = uuid4()
    ...
    assert OrderReviews(postgres_engine).get(order_check_id) == order_check
...
def test_restore_installs_new_module_tables_without_replacing_existing_facts(
    ...
    with _empty_restore_database(postgres_engine) as source:
        ...
        assert (
            LiveStreams(source, DataLibrary(source, SourceFiles(tmp_path / "sources"))).verify_all()
            == 0
        )
...
def test_missing_order_check_parent_keeps_restore_unactivated(
    ...
    comparison = OrderReviews(postgres_engine).check(position_check_id, request_id=order_check_id)
