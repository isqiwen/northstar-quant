...
def test_partial_order_remains_active_when_cancel_is_submitted_or_rejected(
    ...
    result = OrderReviews(postgres_engine).check(parent, request_id=uuid4())
...
def test_entries_created_after_fixed_parent_never_pollute_its_order_comparison(
    ...
    command = uuid4()
    result = OrderReviews(postgres_engine).check(parent, request_id=command)
    assert result["status"] == "MATCHED"
    ...
    assert OrderReviews(postgres_engine).get(command) == result
...
def test_order_review_is_idempotent_immutable_and_checks_its_fixed_parent_evidence(
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(
            executor.map(
                lambda _: OrderReviews(postgres_engine).check(parent, request_id=command), range(2)
            )
        )
    assert results[0] == results[1] == OrderReviews(postgres_engine).get(command)
    other_parent = uuid4()
    ledger.compare(UUID(entry["entry_id"]), ledger_query(postgres_engine), request_id=other_parent)
    with pytest.raises(ValueError, match="bound"):
        OrderReviews(postgres_engine).check(other_parent, request_id=command)
    with pytest.raises(ValueError, match="already has"):
        OrderReviews(postgres_engine).check(parent, request_id=uuid4())
    ...
    with pytest.raises(ValueError, match="damaged"):
        OrderReviews(postgres_engine).get(command)
    with postgres_engine.begin() as connection:
        connection.execute(text("SET LOCAL session_replication_role = replica"))
        connection.execute(
