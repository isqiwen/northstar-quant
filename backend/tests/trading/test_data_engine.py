...
def test_streams_route_independently_and_failed_candidates_preserve_all_views():
    ...
    other = replace(data.bars[0], close=Decimal("200"), contract_id=second.contract_id)
    ...
    with pytest.raises(ValueError, match="different facts"):
        engine.advance(first, replace(other, contract_id=first.contract_id), at=other.available_at)
    ...
    with pytest.raises(ValueError, match="backwards"):
        engine.advance(
            second,
            replace(data.bars[1], contract_id=second.contract_id),
            at=data.bars[1].available_at,
        )
...
def test_wrong_contract_cannot_enter_empty_stream_or_saved_warmup():
    from northstar_quant.market_data.window import MarketWindow

    data = dataset(("100", "103"))
    stream = BarStream(data.market.contract_id, 60)
    wrong = replace(data.bars[0], contract_id=uuid4())
    engine = DataEngine().subscribe(stream, 2)
    with pytest.raises(ValueError, match="different contract"):
        engine.advance(stream, wrong, at=wrong.available_at)
    assert engine.window(stream).bars == ()
    with pytest.raises(ValueError, match="different contract"):
        DataEngine().subscribe(stream, 2, history=(wrong,))
    with pytest.raises(ValueError, match="different contract"):
        MarketWindow(2, 60, (wrong, data.bars[1]))
    with pytest.raises(ValueError, match="different contract"):
        replace(data, bars=(wrong,))
