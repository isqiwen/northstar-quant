...
def test_request_identity_and_retries_keep_fixed_evidence_and_distinct_attempts(
    ...
    assert len(files.inventory()) == 1
    monkeypatch.setattr(
        "northstar_quant.data_management.library.code_revision", lambda: "a" * 40 + "-dirty"
    )
    development = library.reprocess(
        UUID(str(first["source_id"])), spec=spec, request_id=str(uuid4())
    )
    assert development["reused_product"] is False
    next_development = library.reprocess(source_id, spec=spec, request_id=str(uuid4()))
    assert next_development["reused_product"] is False
    assert next_development["snapshot_id"] != development["snapshot_id"]
...
def test_unavailable_archive_blocks_download_research_and_new_paper(
    ...
    sessions = PaperStore(postgres_engine, library)
    saved = ConfigurationStore(postgres_engine).save_configuration("No damaged input", config)
    with pytest.raises(ValueError):
...
def test_converted_input_tracks_actual_upstream_and_both_run_consumers(
    ...
    run_id = RunStore(postgres_engine).save(dataset, config, run_research(dataset, config))
    sessions = PaperStore(postgres_engine, library)
    saved = ConfigurationStore(postgres_engine).save_configuration("Fixed source", config)
    session_id = uuid4()
...
def test_interrupted_publication_is_not_offered_and_retry_recovers_without_duplicate_facts(
    ...
    with postgres_engine.connect() as connection:
        ...
        assert connection.scalar(text("SELECT count(*) FROM dataset_snapshot_manifest")) == (
            2 if dirty else 1
        )
