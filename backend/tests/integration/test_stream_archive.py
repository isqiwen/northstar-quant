...
def test_paused_shadow_source_publishes_without_rewriting_decisions_and_reuses_after_restart(
    ...
    paper = PaperStore(postgres_engine, library)
    saved_configuration = ConfigurationStore(postgres_engine).save_configuration(
        "archived samples", configuration_value
    )
    paper_id = uuid4()
...
def test_archive_rejects_forged_permission_and_does_not_truncate_oversized_prefix(
    ...
    monkeypatch.setattr(stream_records, "_ARCHIVE_BYTES", 1)
...
def test_browser_archive_requires_csrf_and_publishes_saved_prefix_without_connecting(
    ...
    with TestClient(live_web_app(postgres_engine, library), base_url="http://127.0.0.1") as client:
        ...
        assert client.get(f"/attempts/{attempt['attempt_id']}").status_code == 404
        assert client.get(f"/datasets/{attempt['snapshot_id']}").status_code == 404
        ...
        assert client.get(f"/attempts/{failure.json()['attempt_id']}").status_code == 404
