...
from northstar_quant.broker.ledger import _hash, _time
...
class _StreamAccount:
    ...
    def _read(connection: Connection, stream_id: UUID) -> dict[str, Any]:
        ...
        row = (
            connection.execute(
                text("""
                SELECT s.received, s.binding_hash AS source_binding_hash,
                ...
                FROM broker_streams s LEFT JOIN broker_stream_accounts a USING (stream_id)
    ...
    def bind(self, baseline_id: UUID, stream_id: UUID) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            ...
            source = cast(dict[str, Any], BrokerStreams._row(connection, stream_id)["binding"])
