    ...
    def _request(
        ...
        *,
        ...
        if response.status_code in {400, 403, 405, 409, 413, 422}:
            raise ValueError("Live rejected permission, current release, target or command input")
        if response.status_code != 200:
            raise RuntimeUnavailable("Live did not confirm the operation")
        if response.headers.get("x-northstar-release") != release_hash():
            raise RuntimeUnavailable("Live does not match the current release")
        try:
            value = response.json()
