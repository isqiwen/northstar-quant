...
_OPEN_STATUSES = {'submitted', 'presubmitted', 'pending', 'open', 'Submitted', 'PreSubmitted', 'PendingSubmit'}
...
def cancel_stale_orders(session: Session, broker: BrokerAdapter) -> dict:
    ...
    session.commit()
    return {'stale_order_count': len(rows), 'canceled_order_ids': canceled_ids}
