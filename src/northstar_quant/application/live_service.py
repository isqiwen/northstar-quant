...
from northstar_quant.trading_execution.execution.registry import build_execution_plan, resolve_execution_planner
from northstar_quant.trading_execution.execution.router import OrderRouter
from northstar_quant.trading_execution.orders.order_management import cancel_stale_orders
...
from northstar_quant.trading_execution.reconciliation.reconciliation import analyze_position_drift, reconcile_broker_state
from northstar_quant.trading_execution.live.runtime_risk import (
    RuntimeRiskAssessment,
    assess_runtime_risk,
    runtime_risk_symbols,
)
from northstar_quant.application.target_service import (
...
from northstar_quant.application.reporting import latest_live_account_attribution_summary
from northstar_quant.portfolio_risk.limits.models import OrderRiskContext, SymbolTradeState
from northstar_quant.portfolio_risk.risk.pretrade import reserve_open_orders_in_context
...
def _assert_live_submission_allowed(
    broker_name: str,
    _order: OrderRequest,
) -> None:
    """在每次券商提交前重新读取安全开关并执行最后一道门禁。

    日历制品的来源授权可能在一个订单批次中被撤销或换版，因此不能复用批次开始时缓存的
    ``CalendarService``。每笔订单都重新读取并验证 immutable artifact、当前来源配置和授权。
    """
    ...
    # 画像本身可能在长进程存活期间被操作员撤销或替换日历/合约制品 pin。最后一道
    # 提交门禁不能复用研究与 CLI 使用的进程缓存；每笔非 paper 订单都重新解析文件。
    profile = load_trading_profile_uncached(_order.profile_id)
    try:
        # 不只重读日历 pin：画像生命周期、实际合约模式和数据治理资格也可能在批次执行中
        # 被操作员撤销。它们必须在读取运行时风控、Contract Master 或日历前再次失败关闭。
        validate_profile_data_governance(profile)
        ...
        raise PermissionError(
            "PROFILE_GOVERNANCE_BLOCKED: 当前画像不再满足券商、生命周期或数据治理提交条件。"
        ) from exc
    ...
    # 必须在 prepare_order() 后用最终实际月份合约重新验证 Contract Master。日历门禁只
    # 证明品种会话，不能替代挂牌、到期、规则质量或交割限制。
    submission_at = utc_now()
    _contract, contract_rules = assert_execution_contract_admissible(
        profile=profile,
    ...
    assert_order_calendar_open(
        ...
        broker_name=normalized_broker,
        order=_order,
        calendar_service=load_calendar_service_for_profile(profile),
        contract_rule_snapshot=contract_rules,
        at=submission_at,
    )
...
def _collect_execution_symbols(
    ...
    *,
    ...
    if profile.asset_type == AssetType.FUTURES:
        ...
        if any(str(symbol).strip().upper().endswith("_CONT") for symbol in output_symbols):
            raise ValueError(
