...
def reconcile_broker_state(session: Session, broker: BrokerAdapter) -> dict:
    """同步券商状态并写入数据库。
    ...
    write_sync_log(
        ...
        status='success',
        detail=f'positions={pos_count}, fills={fill_count}, open_orders={len(snapshot.open_orders)}, updated_orders={updated_orders}',
    )
    reconcile_logger.info(
        "券商状态同步完成，positions=%s，fills=%s，open_orders=%s，updated_orders=%s",
        pos_count,
        fill_count,
